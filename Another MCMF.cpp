...
pair <ll, ll> minCostMaxFlow(int n, int s, int t, ll k) {
...
ll killAttacking(int n, int m, int s, int t) {
	...
	ll ans = 0;
	for (ll flow = 1; flow <= max(n, m); ++flow) {
		...
		for (int i = m + 1; i <= n + m; ++i) edges.push_back((edge){i, t, 1, 0});
         	...
         	ans = max(ans, -minCostMaxFlow(n + m + 2, s, t, flow).se);
	}
	return ans;
}
...
ll killAll(int n, int m) {
	...
	auto F = minCostMaxFlow(2 * m + 2, s, t, inf);
