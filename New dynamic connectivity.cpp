...
void solve() {
	...
	for (int l = 0; l < m; l += block) {
		...
		vector <int> del(m);
		for (int i = l; i <= r; ++i) { // edges that will remain through the whole block
			int t, v, u; tie(t, v, u) = q[i];
			if (t == 3) del[v] = 1;
		}

		for (int i = 0; i < l; ++i) { // building DSU on edges that will remain through the whole block
			int t, v, u; tie(t, v, u) = q[i];
		...
		for (int i = l; i <= r; ++i) {
			...
			if (t == 2) global[i] = 1;
				...
				if (par(v) == par(u)) { // first try to find the answer in our DSU
					cout << "YES" << '\n';
					continue;
				}

				unordered_set <int> li; li.insert(par(v)); li.insert(par(u)); // if not, then we will have at most "block" additional edges in our current bucket
				for (int j = l; j <= r; ++j) {
